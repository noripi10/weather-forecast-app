...
import { ActivityIndicator, StyleSheet, Text, Modal, Dimensions, View, Image } from 'react-native';
...
export const LocationScreen = () => {
    ...
    return (
      <View style={styles.noPermissionContainer}>
        ...
        <View style={styles.buttonContainer}>
          <AppButton title='アプリ設定画面を開く' onPress={() => openSetting()} color={colorList.grey3} />
          <AppButton title='再起動' onPress={() => restartApp()} color={colorList.primary} />
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      <AppHeader title='現在地' iconString='md-reload-outline' rightPressHandle={getGeoLocation} />

      <View style={styles.locationInfoContainer}>
        <Text style={{ fontSize: 18, color: colors.text }}>{locationInfo && locationInfo.city}</Text>
      </View>

      {locationInfo.forecastTodayLast && (
        <View style={styles.currentWeatherContainer}>
          <Card containerStyle={[styles.weatherCard, { backgroundColor: colors.card }]}>
            <Card.Title style={{ color: colors.text }}>これからの天気</Card.Title>
            <Card.Divider />

            <View style={[styles.cardItemContainer]}>
              <View style={styles.cardLeftItemContainer}>
                <Text style={{ color: colors.text }}>{locationInfo.forecastTodayLast.weather[0].description}</Text>
                {/* <LottieView
              ...
              <View style={styles.cardRightItemContainer}>
                <Text style={[styles.text, { color: colors.text }]}>
                  {`${moment(locationInfo.forecastTodayLast.dt_txt).format('M/D (ddd) HH:mm')} 時点`}
                </Text>
                <Text style={[styles.text, { color: colors.text }]}>
                  {`気温：${Math.round((locationInfo.forecastTodayLast.main.temp - 273.15) * 100) / 100}℃`}
                </Text>
                <Text
                  style={[
                    styles.text,
          ...
          <Card containerStyle={[styles.weatherCard, { height: '65%', backgroundColor: colors.card }]}>
            <Card.Title style={{ color: colors.text }}>3時間ごとの天気</Card.Title>
            <Card.Divider />
            <View style={[styles.cardItemContainer, { height: '86%', flexDirection: 'column' }]}>
